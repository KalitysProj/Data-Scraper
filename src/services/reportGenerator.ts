...
class AnalysisService {
  ...
  async analyzeWebsite(url: string, onProgress?: (progress: number, status: string) => void): Promise<AnalysisServiceResult> {
    try {
      ...
      const analysisResult = reportGenerator.generateReport(realData, userId);
  ...
  private generateAccessibilityIssues(seoData: RealAnalysisResult['seoData'], url: string): AccessibilityIssue[] {
    ...
    if (imagesWithoutAlt.length > 0) {
      issues.push({
        type: 'missing_alt_text',
        severity: 'medium',
        message: `${imagesWithoutAlt.length} image(s) sans texte alternatif`,
        element: 'img',
        wcagCriterion: '1.1.1',
        solution: 'Ajouter des attributs alt descriptifs pour l\'accessibilité',
        url
      });
    }
  ...
  private generateSEORecommendations(seoData: RealAnalysisResult['seoData']): string[] {
    ...
    const imagesWithoutAlt = seoData.images.filter(img => !img.hasAlt);
    if (imagesWithoutAlt.length > 0) {
      recommendations.push('🖼️ Optimiser les images : Ajouter des attributs alt à toutes les images');
    }
    ...
    try {
      const urlObj = new URL(seoData.url || '');
